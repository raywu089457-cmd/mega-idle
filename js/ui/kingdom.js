...
MG.ui.kingdom = (function () {
  ...
  function drawTownLife(t) {
    ...
    // v845 圖書館講台（錨 library[144,116] 簷側 — 木台＋開書；建成才畫；rm 定幀）
    {
      if ((st.buildings.library || 0) > 0) {
        const lx = 190, ly = 152;
        fxCtx.fillStyle = "#6a4a28";
        fxCtx.fillRect(lx, ly + 6, 12, 3); // 台
        fxCtx.fillStyle = "#8a6238";
        fxCtx.fillRect(lx + 1, ly + 4, 10, 2); // 面
        fxCtx.fillStyle = "#e8dcc0";
        fxCtx.fillRect(lx + 2, ly + 1, 8, 4); // 開書
        fxCtx.fillStyle = "#3a5a9a";
        fxCtx.fillRect(lx + 5, ly + 1, 1, 4); // 書脊線
      }
    }
    // v845 訓練木人樁（錨 training[208,58] 場側 — 褐樁＋草繩；建成才畫；rm 定幀）
    {
      if ((st.buildings.training || 0) > 0) {
        const tx = 248, ty = 118;
        fxCtx.fillStyle = "#8a6238";
        fxCtx.fillRect(tx + 3, ty + 2, 5, 12); // 樁
        fxCtx.fillStyle = "#6a4a28";
        fxCtx.fillRect(tx + 1, ty + 12, 9, 2); // 座
        fxCtx.fillStyle = "#57c96b";
        fxCtx.fillRect(tx + 2, ty + 5, 7, 2); // 草繩
        fxCtx.fillStyle = "#c8a878";
        fxCtx.fillRect(tx + 4, ty + 3, 2, 4); // 亮
      }
    }
    // v845 市場天平（錨 market[366,116] 簷前 — 銅臂＋盤；建成才畫；rm 定幀）
    {
      if ((st.buildings.market || 0) > 0) {
        const mx = 402, my = 152;
        fxCtx.fillStyle = "#d4a84a";
        fxCtx.fillRect(mx + 5, my, 2, 8); // 柱
        fxCtx.fillRect(mx + 1, my + 2, 10, 2); // 臂
        fxCtx.fillStyle = "#c8a878";
        fxCtx.fillRect(mx, my + 4, 4, 3); // 盤L
        fxCtx.fillRect(mx + 8, my + 4, 4, 3); // 盤R
        fxCtx.fillStyle = "#57c96b";
        fxCtx.fillRect(mx + 1, my + 5, 2, 1); // 貨L
        fxCtx.fillStyle = "#e05c5c";
        fxCtx.fillRect(mx + 9, my + 5, 2, 1); // 貨R
      }
    }
    // v849 酒館酒桶（錨 guild[134,58] 簷側 — 褐桶＋泡沫；建成才畫；rm 定幀）
    {
      if ((st.buildings.guild || 0) > 0) {
        const gx = 168, gy = 105;
        const foam = !rm && ((t * 2.0) % 2.1) < 0.28;
        fxCtx.fillStyle = "#8a6238";
        fxCtx.fillRect(gx + 1, gy + 2, 10, 10); // 桶
        fxCtx.fillStyle = "#6a6a78";
        fxCtx.fillRect(gx, gy + 4, 12, 2); // 箍
        fxCtx.fillRect(gx, gy + 9, 12, 2);
        fxCtx.fillStyle = "#c8a878";
        fxCtx.fillRect(gx + 4, gy + 3, 2, 8); // 亮
        if (foam || rm) {
          fxCtx.globalAlpha = rm ? 0.35 : 0.7;
          fxCtx.fillStyle = "#fff8e8";
          fxCtx.fillRect(gx + 3, gy + 1, 6, 2);
          fxCtx.globalAlpha = 1;
        }
      }
    }
    // v849 煉金研缽（錨 alchemy[70,116] 簷前 — 石缽＋杵；建成才畫；rm 定幀）
    {
      if ((st.buildings.alchemy || 0) > 0) {
        const ax = 108, ay = 138;
        fxCtx.fillStyle = "#8a8a98";
        fxCtx.fillRect(ax + 1, ay + 5, 10, 5); // 缽
        fxCtx.fillStyle = "#6a6a78";
        fxCtx.fillRect(ax, ay + 4, 12, 2); // 緣
        fxCtx.fillStyle = "#8a6238";
        fxCtx.fillRect(ax + 8, ay, 2, 8); // 杵
        fxCtx.fillStyle = "#57c96b";
        fxCtx.fillRect(ax + 3, ay + 6, 4, 2); // 藥
      }
    }
    // v849 寶石放大鏡（錨 gemworks[356,58] 簷前 — 銅圈＋柄；建成才畫；rm 定幀）
    {
      if ((st.buildings.gemworks || 0) > 0) {
        const gx = 378, gy = 128;
        const gleam = !rm && ((t * 2.4) % 2.0) < 0.26;
        fxCtx.fillStyle = "#d4a84a";
        fxCtx.fillRect(gx + 7, gy + 5, 5, 2); // 柄
        fxCtx.strokeStyle = "#d4a84a";
        fxCtx.lineWidth = 2;
        fxCtx.beginPath();
        fxCtx.arc(gx + 4, gy + 4, 4, 0, Math.PI * 2);
        fxCtx.stroke();
        fxCtx.fillStyle = "#7ec8ff";
        fxCtx.fillRect(gx + 2, gy + 2, 4, 4); // 鏡
        if (gleam || rm) {
          fxCtx.globalAlpha = rm ? 0.35 : 0.7;
          fxCtx.fillStyle = "#ffffff";
          fxCtx.fillRect(gx + 3, gy + 3, 1, 1);
