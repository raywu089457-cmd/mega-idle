    ...
    for (const e of evs) {
      ...
      switch (e.type) {
        ...
        case "skill": {
          ...
          if (sk.type === "taunt") spawnTauntMark(310, 200); // v719：嘲諷紅標（怪物側）
          if (sk.type === "multi") spawnMultiMark(310, 188); // v723：連擊橘疊 V
          if (sk.type === "heal") spawnHealRing(hx, hy); // v723：治療綠雙環
          if (fx === "fx_poison") spawnPoisonMark(310, 200); // v723：淬毒紫六角
          if (sk.type === "buff") spawnBuffMark(hx, hy); // v727：增益金盾標
          ...
          if (fx === "fx_slash") spawnSlashMark(310, 200); // v731：斬擊銀灰弧
          if (fx === "fx_arrow") spawnArrowMark(310, 200); // v734：箭矢金羽
          if (fx === "fx_dagger") spawnDaggerMark(310, 200); // v734：匕首銀叉
          if (fx === "fx_shield") spawnShieldMark(hx, hy); // v734：護盾藍盾（英雄側）
          if (sk.crit) spawnCritSkillMark(310, 188); // v738：必暴金星
          if (sk.heal && sk.type === "hit") spawnLeechMark(hx, hy); // v738：吸血綠心（攻擊技回血）
          if (sk.freeze) spawnChillMark(310, 200); // v738：凍結雪晶
          if ((sk.power || 0) >= 2.5) spawnPowerMark(310, 188); // v742：重擊橙菱
          if (sk.type === "multi" && (sk.hits || 0) >= 4) spawnRapidMark(310, 188); // v746：高連擊青速線
          if (sk.dot && fx === "fx_fireball") spawnBurnMark(310, 188); // v750：灼燒火尖
          if (sk.dot && fx === "fx_poison") spawnToxCast(310, 188); // v754：毒 DoT 施放滴尖
