...
MG.sys.hunters = (function () {
  ...
  function artifactAwakenCost(aid) {
    ...
    // v796：加深軟封頂 min(aw,0) — aw≤0 不變；二／三覺加深平坦
  ...
  function artifactRefineCost(aid) {
    ...
    // v800：加深軟封頂 min(lv-4,0) — lv≤4 不變；高階精煉加深平坦
  ...
  function refinePreview(aid) {
    ...
    while (lv < 10) {
      // v696FIX／v712／v732／v748／v752／v800：與 artifactRefineCost 同源（含 deepen 軟封）— 預覽不再低估／高估
      let stepGold = 400 * Math.pow(lv, 1.6);
      if (lv >= 5) stepGold *= Math.pow(1.2, Math.min(lv - 4, 0));
  ...
  /* v676：徽章升階金幣單一來源（UI 顯示與 badgeUp 實扣同源）
     ...
     v732：加深軟封頂 min(lv-2,2) — lv≤4 不變；防 5–6 階牆
     v748：加深軟封頂 min(lv-2,1) — lv≤3 不變；防 4–6 階牆
     v808：加深軟封頂 min(lv-2,0) — lv≤2 不變；4–6 階 deepen 歸零（flat） */
  function badgeGoldCost(lv) {
    let gold = 300 * Math.pow(2, lv);
    if (lv >= 3) gold *= Math.pow(1.25, Math.min(lv - 2, 0));
