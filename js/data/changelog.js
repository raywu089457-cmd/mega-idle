...
MG.data.changelog = [
  {
    v: "v172", title: "村莊東擴＋建築放大貼地＋TheoTown 小人",
    notes: [
      "村莊東擴 14×14→18×14（c1 13→17）：草原區/農田東移、森林北移、東城門外道路重接，可容納更大建築與更寬街道",
      "建築放大 1.15-1.4 倍（公會/訓練場/圖書館/鐵匠鋪/煉金坊/市集 1.4、祭壇 1.2、寶石坊/倉庫 1.15、城堡 1.2），底邊貼地錨點＋陰影貼腳（不再懸空）",
      "角色全部改為 TheoTown 風格小人：圓頭＋髮色＋職業色身體＋暗描邊＋交替步伐＋貼地小影子（職業色取自職業資料）",
      "街道網配合東擴：東街移至 c15.5、中街貫通至東城門（c17）、南巷移至 c10.5"
    ]
  },
  {
    v: "v171", title: "村鎮重構：全建築入城＋街道網＋全角色上街",
    notes: [
      "9 棟功能建築全部恆常建造入城（不再依已建等級隱藏），妥善沿街排開：圖書館/祭壇靠西街、公會/訓練場靠東街、鐵匠鋪/煉金坊臨中街、寶石坊/市集/倉庫臨南街，城堡居北正對北城門，民房在西街西側住宅區",
      "完整街道網：西街/東街/北街/中街（東西城門貫通）/南街/南巷/北巷，街道與行走圖 VEDGES 完全一致",
      "所有角色都走在街道上：全體領地英雄（職業 sprite）＋全體流浪英雄＋2 村民，隨機目標節點＋BFS 最短路徑行走，到點暫停再出發；「減少動畫效果」時定幀",
      "城門缺口、水井、路樹位置同步調整；東城門外道路與中街相接"
    ]
  },
  {
    v: "v170", title: "村外農田：圍籬麥田＋稻草人＋乾草堆＋麥浪",
    notes: [
      "村莊東南方新增圍籬麥田：泥土田壟＋北東側木樁圍籬＋稻草人（木桿/草帽/紅衣/雙臂）＋乾草堆",
      "36 簇麥穗隨風搖曳（每簇 3 根，相位錯開的麥浪）；「減少動畫效果」時定幀",
      "世界地圖的 TheoTown 鄉村感：城鎮＋農田＋野生魔物＋路上馬車"
    ]
  },
  {
    v: "v169", title: "區域野生怪物：在地魔物地標旁遊蕩",
    notes: [
      "每個已解鎖區域有 2 隻在地原生魔物（如草原史萊姆＋野狼、洞穴巨鼠＋岩石巨魔、火山火蜥蜴＋熔岩元素、神話墮天使＋星獸）沿小橢圓路徑在地標旁遊蕩，走路動畫＋隨行進方向翻轉＋落地陰影",
      "鎖定區迷霧內不顯示；「減少動畫效果」時定幀佇立",
      "世界地圖因此有了 TheoTown 式的生態感：解鎖即見該區魔物棲息"
    ]
  },
  {
    v: "v168", title: "區域地標：十區主題聚落（TheoTown 世界感）",
    notes: [
      "每個已解鎖區域在地圖中心有主題地標：草原風車磨坊（旋轉葉片）、森林獵人小屋（炊煙）、洞穴礦坑口（軌道礦車＋火把）、火山黑曜祭壇（烈焰火盆＋熔岩脈動）、冰原冰晶祭壇（閃爍）、荒漠綠洲帳棚（水光＋棕櫚）、沼澤巫婆高腳屋（藥鍋泡泡）、蒼穹瞭望塔（旗飄）、深淵裂谷哨站（燈籠搖曳＋懸崖木柵）、神話遺跡拱門（浮球脈動）",
      "擊敗區域守關 BOSS 後地標升級（金色飾邊/燈塔信標/金礦/鹿角裝飾等），解鎖進度一眼可見",
      "未解鎖區域迷霧內不繪製地標（不露餡）；「減少動畫效果」時全部定幀靜止",
      "區域名牌上移至地標上方"
    ]
  },
  {
    v: "v167", title: "村莊擴建：全建築入城＋主街佈局＋英雄路人走動＋蜿蜒道路",
    notes: [
      "村莊範圍擴大 8×8→14×14：9 棟功能建築全部入城沿主街兩側排開（南北主街/東西主街），中央廣場城堡、四城門、角塔、住宅區民房、路樹水井，相對大小合理",
      "外部道路蜿蜒化：每段插入中間點＋fbm 垂直偏移（±1.2 tile），馬車同步改行蜿蜒路線",
      "村內活人走動：已招募英雄（職業 sprite 走路動畫）＋村民路人沿道路環線漫步（16 秒一圈）；「減少動畫效果」時定點佇立",
      "草原區中心東移避讓擴大村莊；道路起點改村莊東門"
    ]
  },
  {
    v: "v166", title: "地圖活化：海洋波紋／雲影／路上馬車／煙囪冒煙／熔岩脈動",
    notes: [
      "世界地圖加入 TheoTown 風動態層：海洋波紋流動閃爍、兩片雲影緩慢右飄、小貨車沿道路村莊↔已解鎖區往返（42 秒單程）",
      "鐵匠鋪煙囪持續冒煙（三縷上升飄散）；烈焰火山熔岩縫明暗脈動（解鎖後才顯示）",
      "「減少動畫效果」開啟時馬車定點佇立、其餘動態全靜止",
      "修正地圖畫布縮放（dpr>1 裝置不再放大裁切，邏輯目標固定 VW×VH）"
    ]
  },
  {
