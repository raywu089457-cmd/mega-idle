...
1. 讀取:progress/improvement-log.md(狀態行確認本輪軌道與 backlog)、docs/DESIGN.md §5/§2、上表檔案地圖中相關檔案;
   index.html(快取版號)
2. 取證確認(K3 規劃閘門已選題,見 @progress/round-<R>-plan.md):瀏覽器以行動視口實際走主要路徑,
   確認 plan 所指摩擦點真實存在並量測改動前現況(點擊序列/時間/截圖) —
   若取證推翻 plan,在報告標註,但仍以 plan 為主
3. 實作:依 round-<R>-plan.md 的【方案】實作(可微調互動細節,不換題/不縮水);嚴禁大重構
...
6. 收尾:js/data/changelog.js 加 vN 條目、index.html 快取 +1、
   ...
   **不更動狀態行** — 輪次/下一軌道由觸發器(loop-trigger.js)在 K3 評審通過後推進
   (本輪會被 prompts/goal-judge.md 的 K3 評審複審:報告與證據[摩擦量測/流程截圖/斷言]須自足,
