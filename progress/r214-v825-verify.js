/* v825 village-art ×3 verify */
"use strict";
const fs = require("fs");
const path = require("path");
const { chromium } = require("C:/Users/ray/AppData/Roaming/npm/node_modules/playwright");
const OUT = path.join(__dirname);
const TAG = "round-214-v825";
...
(async () => {
  ...
  await page.goto("http://127.0.0.1:8123/index.html?v=825", { waitUntil: "domcontentloaded" });
  ...
  const r = await page.evaluate(async () => {
    ...
    const stool = countColor(143, 98, 157, 114, (r, g, b) =>
      (r > 100 && g > 60 && b < 90) || (r > 160 && g > 120 && b < 120));
    const rack = countColor(93, 148, 107, 164, (r, g, b) =>
      (g > r && g > 100 && b < 140) || (r > 100 && g > 60 && b < 90));
    const pedestal = countColor(383, 106, 397, 120, (r, g, b) =>
      (b > r && b > 120) || (r > 160 && g > 120 && b < 120));

    st.settings.reducedMotion = true;
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h1a = hashRoi(143, 98, 157, 114);
    const h1b = hashRoi(93, 148, 107, 164);
    const h1c = hashRoi(383, 106, 397, 120);
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h2a = hashRoi(143, 98, 157, 114);
    const h2b = hashRoi(93, 148, 107, 164);
    const h2c = hashRoi(383, 106, 397, 120);

    return { stool, rack, pedestal, rmSame: h1a === h2a && h1b === h2b && h1c === h2c };
  });
  ...
  const asserts = [
    { name: "srcStool", ok: k.includes("v825 酒館木凳") },
    { name: "srcRack", ok: k.includes("v825 煉金藥架") },
    { name: "srcPedestal", ok: k.includes("v825 寶石托座") },
    { name: "stoolROI", ok: r.stool >= 6 },
    { name: "rackROI", ok: r.rack >= 6 },
    { name: "pedestalROI", ok: r.pedestal >= 6 },
