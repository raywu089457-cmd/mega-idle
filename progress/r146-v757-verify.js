/* v757 village-art ×3 verify */
"use strict";
const fs = require("fs");
const path = require("path");
const { chromium } = require("C:/Users/ray/AppData/Roaming/npm/node_modules/playwright");
const OUT = path.join(__dirname);
const TAG = "round-146-v757";
...
(async () => {
  ...
  await page.goto("http://127.0.0.1:8123/index.html?v=757", { waitUntil: "domcontentloaded" });
  ...
  const r = await page.evaluate(async () => {
    ...
    // 木梯褐
    const ladder = countColor(232, 142, 246, 156, (r, g, b) =>
      r > 90 && g > 50 && b < 80 && r >= g && g > b);
    // 香火紫
    const incense = countColor(328, 142, 342, 156, (r, g, b) =>
      b > r && b > 140 && g > 80 && g < 200);
    // 果籃紅／橙
    const basket = countColor(372, 146, 388, 160, (r, g, b) =>
      r > 180 && g < 160 && b < 140);

    st.settings.reducedMotion = true;
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h1 = hashRoi(230, 140, 390, 162);
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h2 = hashRoi(230, 140, 390, 162);

    return { ladder, incense, basket, rmSame: h1 === h2, h1, h2 };
  });
  ...
  assert("srcLadder", k.includes("v757 倉庫木梯"));
  assert("srcIncense", k.includes("v757 祭壇香火"));
  assert("srcBasket", k.includes("v757 市場果籃"));
  assert("ladderROI", r.ladder >= 6);
  assert("incenseROI", r.incense >= 3);
  assert("basketROI", r.basket >= 4);
  ...
  const table = [
    "=== 村莊美術驗證 ===",
    "ladder=" + r.ladder + " incense=" + r.incense + " basket=" + r.basket + " rmSame=" + r.rmSame,
