/* v817 village-art ×3 verify */
"use strict";
const fs = require("fs");
const path = require("path");
const { chromium } = require("C:/Users/ray/AppData/Roaming/npm/node_modules/playwright");
const OUT = path.join(__dirname);
const TAG = "round-206-v817";
...
(async () => {
  ...
  await page.goto("http://127.0.0.1:8123/index.html?v=817", { waitUntil: "domcontentloaded" });
  ...
  const r = await page.evaluate(async () => {
    ...
    const broom = countColor(263, 146, 276, 164, (r, g, b) =>
      (r > 100 && g > 60 && b < 90) || (r > 160 && g > 120 && b < 120));
    const tongs = countColor(318, 120, 330, 136, (r, g, b) =>
      (Math.abs(r - g) < 30 && r > 90 && r < 180) || (r > 100 && g > 60 && b < 90));
    const candle = countColor(333, 136, 345, 152, (r, g, b) =>
      (r > 180 && g > 160 && b > 140) || (r > 160 && g > 100 && b < 100) || (r > 160 && g > 120 && b < 120));

    st.settings.reducedMotion = true;
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h1a = hashRoi(263, 146, 276, 164);
    const h1b = hashRoi(318, 120, 330, 136);
    const h1c = hashRoi(333, 136, 345, 152);
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h2a = hashRoi(263, 146, 276, 164);
    const h2b = hashRoi(318, 120, 330, 136);
    const h2c = hashRoi(333, 136, 345, 152);

    return { broom, tongs, candle, rmSame: h1a === h2a && h1b === h2b && h1c === h2c };
  });
  ...
  const asserts = [
    { name: "srcBroom", ok: k.includes("v817 倉庫掃帚") },
    { name: "srcTongs", ok: k.includes("v817 鐵匠鉗") },
    { name: "srcCandle", ok: k.includes("v817 祭壇香燭") },
    { name: "broomROI", ok: r.broom >= 6 },
    { name: "tongsROI", ok: r.tongs >= 6 },
    { name: "candleROI", ok: r.candle >= 6 },
