/* v801 village-art ×3 verify */
"use strict";
const fs = require("fs");
const path = require("path");
const { chromium } = require("C:/Users/ray/AppData/Roaming/npm/node_modules/playwright");
const OUT = path.join(__dirname);
const TAG = "round-190-v801";
...
(async () => {
  ...
  await page.goto("http://127.0.0.1:8123/index.html?v=801", { waitUntil: "domcontentloaded" });
  ...
  const r = await page.evaluate(async () => {
    ...
    const flask = countColor(96, 136, 112, 152, (r, g, b) =>
      (g > r && g > 80 && b < 140) || (r > 100 && g > 60 && b < 80));
    const sack = countColor(244, 132, 264, 150, (r, g, b) =>
      (r > 140 && g > 120 && b < 140 && r > b) || (r > 100 && g > 70 && b < 80));
    const ink = countColor(171, 134, 188, 152, (r, g, b) =>
      (b > r && b > 70 && g < 140) || (r > 160 && g > 140 && b > 160));

    st.settings.reducedMotion = true;
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h1a = hashRoi(96, 136, 112, 152);
    const h1b = hashRoi(244, 132, 264, 150);
    const h1c = hashRoi(171, 134, 188, 152);
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h2a = hashRoi(96, 136, 112, 152);
    const h2b = hashRoi(244, 132, 264, 150);
    const h2c = hashRoi(171, 134, 188, 152);

    return { flask, sack, ink, rmSame: h1a === h2a && h1b === h2b && h1c === h2c };
  });
  ...
  const asserts = [
    { name: "srcFlask", ok: k.includes("v801 煉金燒瓶") },
    { name: "srcSack", ok: k.includes("v801 倉庫糧袋") },
    { name: "srcInk", ok: k.includes("v801 圖書館墨水瓶") },
    { name: "flaskROI", ok: r.flask >= 6 },
    { name: "sackROI", ok: r.sack >= 6 },
