(() => {
  ...
  function dump(sec, label) {
    ...
    const events = (F.events || []).slice(-5).map(e => e.type + (e.fallback ? '->' + (e.fallback.type||'') : ''));
    log.push({ label, sec, stage: st.hunt.stage, phase: F.phase, kills: st.stats.kills, gold: st.currencies.gold, ids: (st.hunt.dispatchIds||[]).length, rest: st.hunt.restUntil > simNow ? Math.round((st.hunt.restUntil - simNow)/1000) : 0, ws: st.hunt.wipeStreak, mhp: F.m ? Math.round(F.hp) : null, aa: st.hunt.autoAdvance, hs: st.hunters.map(h=>h.cls+'L'+h.level).join(',') });
  }
  dispatchAll();
  MG.sys.game.tick(simNow);
  dump(0, 'start');
  for (let s = 60; s <= 1200; s += 60) {
    simNow += 60000;
    MG.sys.game.tick(simNow);
