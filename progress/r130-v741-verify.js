/* v741 village-art ×3 verify */
"use strict";
const fs = require("fs");
const path = require("path");
const { chromium } = require("C:/Users/ray/AppData/Roaming/npm/node_modules/playwright");
const OUT = path.join(__dirname);
const TAG = "round-130-v741";
...
(async () => {
  ...
  await page.goto("http://127.0.0.1:8123/index.html?v=741", { waitUntil: "domcontentloaded" });
  ...
  const r = await page.evaluate(async () => {
    ...
    // 青晶
    const gem = countColor(372, 88, 396, 110, (r, g, b) =>
      b > r && b > g && b > 150 && g > 100);
    // 糧袋米褐
    const sack = countColor(246, 142, 268, 162, (r, g, b) =>
      r > 140 && g > 100 && g < 200 && b < 140 && r > g && g >= b);
    // 風箱皮褐
    const bellows = countColor(306, 90, 328, 112, (r, g, b) =>
      r > 70 && r < 160 && g > 40 && g < 110 && b < 90 && r > g);

    st.settings.reducedMotion = true;
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h1 = hashRoi(230, 85, 400, 165);
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h2 = hashRoi(230, 85, 400, 165);

    return { gem, sack, bellows, rmSame: h1 === h2, h1, h2 };
  });
  ...
  assert("srcGem", k.includes("v741 寶石工坊托盤"));
  assert("srcSack", k.includes("v741 倉庫糧袋"));
  assert("srcBellows", k.includes("v741 鐵匠風箱"));
  assert("gemROI", r.gem >= 4);
  assert("sackROI", r.sack >= 8);
  assert("bellowsROI", r.bellows >= 8);
  ...
  const table = [
    "=== 村莊美術驗證 ===",
    "gem=" + r.gem + " sack=" + r.sack + " bellows=" + r.bellows + " rmSame=" + r.rmSame,
