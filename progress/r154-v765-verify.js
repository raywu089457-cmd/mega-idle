/* v765 village-art ×3 verify */
"use strict";
const fs = require("fs");
const path = require("path");
const { chromium } = require("C:/Users/ray/AppData/Roaming/npm/node_modules/playwright");
const OUT = path.join(__dirname);
const TAG = "round-154-v765";
...
(async () => {
  ...
  await page.goto("http://127.0.0.1:8123/index.html?v=765", { waitUntil: "domcontentloaded" });
  ...
  const r = await page.evaluate(async () => {
    ...
    // 掃帚褐／草
    const broom = countColor(168, 96, 184, 114, (r, g, b) =>
      r > 90 && g > 50 && b < 100 && r >= g);
    // 花束粉／綠
    const flower = countColor(344, 144, 360, 160, (r, g, b) =>
      (r > 180 && g < 160 && b > 120) || (g > r && g > 100 && b < 120));
    // 木箱褐
    const crate = countColor(248, 146, 266, 162, (r, g, b) =>
      r > 90 && g > 50 && b < 90 && r > b);

    st.settings.reducedMotion = true;
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h1a = hashRoi(168, 96, 184, 114);
    const h1b = hashRoi(344, 144, 360, 160);
    const h1c = hashRoi(248, 146, 266, 162);
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h2a = hashRoi(168, 96, 184, 114);
    const h2b = hashRoi(344, 144, 360, 160);
    const h2c = hashRoi(248, 146, 266, 162);
  ...
  assert("srcBroom", k.includes("v765 酒館掃帚"));
  assert("srcFlower", k.includes("v765 祭壇花束"));
  assert("srcCrate", k.includes("v765 倉庫木箱"));
  assert("broomROI", r.broom >= 6);
  ...
  const table = [
    "=== 村莊美術驗證 ===",
    "broom=" + r.broom + " flower=" + r.flower + " crate=" + r.crate + " rmSame=" + r.rmSame,
