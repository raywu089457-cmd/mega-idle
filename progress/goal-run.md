...
## 最後完成輪次: v548（2026-08-15）

### [v548] 改動: 地圖縮放/返回鈕 hover 提示（地圖軸）
理由: 工具列鈕語意不明。
實作:
- js/ui/map.js: zoom 鈕 title（循環倍率）＋返回鈕 title（目的地）
- index.html: 快取 561→562；js/data/changelog.js: v548 條目
驗證:
- 雙 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v548-zoom-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 四軸輪替：動作軸（BOSS 前搖警示強化／精英怪視覺區分）。診斷時打一場副本看特效。

## 前輪: v547（2026-08-15）

### [v547] 改動: 中毒浮字改紫＋毒霧粒子（動作軸）
理由: dot 綠與治療綠混淆 — 扣血/補血一眼難分。
實作:
- js/ui/hunt.js: dot 浮字 #7ac86a→#c792ea ＋ fx_poison 粒子
- index.html: 快取 560→561；js/data/changelog.js: v547 條目
驗證:
- 紫色像素 6 幀取樣（58/236 命中 dot 節奏）；HP 每秒 -14 確認毒傷觸發；完整迴歸通過；rm 靜態；零 console error
- progress/v547-dotpurple.webp
風險與回滾點: 純繪製顏色。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 四軸輪替：地圖軸（道路/村莊互動）或動作軸（BOSS 前搖警示）。診斷時打一場副本看特效。

## 前輪: v546（2026-08-15）

### [v546] 改動: 農田收穫冷卻回饋（地圖軸）
理由: 冷卻中點麥田無回應（靜默 break）。
實作:
- js/ui/map.js: 冷卻中 toast「麥田冷卻中（剩 N 秒）」＋return
- index.html: 快取 559→560；js/data/changelog.js: v546 條目
驗證:
- 源碼確認（拖曳捲動不可驅動 — v420/v440 同款源碼驗證先例）；完整迴歸通過；rm 靜態；零 console error
- progress/v546-farmcd.webp
風險與回滾點: 純 toast 分支。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 四軸輪替：動作軸（BOSS 前搖警示強化／精英怪視覺區分）或
  地圖軸（道路連結視覺／村莊互動）。診斷時打一場副本看特效。
...
理由: 升級成本/效果不明。
實作:
- js/ui/kingdom.js: 10 建築鈕 title（成本・效果預覽）
- index.html: 快取 555→556；js/data/changelog.js: v542 條目
驗證:
- 10 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v542-upbtn-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 263 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v541（2026-08-15）

### [v541] 改動: 戰鬥紀錄列 hover 提示
理由: 紀錄列語意不明。
實作:
- js/ui/hunt.js: 紀錄列 title（最近 8 筆）
- index.html: 快取 554→555；js/data/changelog.js: v541 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v541-log-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 262 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v540（2026-08-15）

### [v540] 改動: 戰利品面板 hover 提示
理由: 掉落率語意不明。
實作:
- js/ui/hunt.js: 戰利品面板 title（掉落基礎值・素材用途）
- index.html: 快取 553→554；js/data/changelog.js: v540 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v540-lootpanel-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 261 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。
...
理由: 練功建議邏輯不明。
實作:
- js/ui/hunt.js: 最佳練功點 title（掃描・收益判定）
- index.html: 快取 550→551；js/data/changelog.js: v537 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v537-farmspot-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 258 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v536（2026-08-15）

### [v536] 改動: 派遣戰力門檻 hover 提示
理由: 戰力比語意不明。
實作:
- js/ui/hunt.js: 出戰隊 vs 建議 title（綠黃紅・指引）
- index.html: 快取 549→550；js/data/changelog.js: v536 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v536-powreq-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 257 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v535（2026-08-15）

### [v535] 改動: BOSS 關預告 hover 提示
理由: BOSS 獎勵規則不明。
實作:
- js/ui/hunt.js: BOSS 預告 title（首殺獎勵・機制建議）
- index.html: 快取 547→548；js/data/changelog.js: v535 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v535-boss-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 256 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v534（2026-08-15）

### [v534] 改動: 副本隊伍列 hover 提示
理由: 隊員狀態缺可讀性。
實作:
- js/ui/hunt.js: 隊員卡 title（HP/MP/狀態）＋空槽引導
- index.html: 快取 546→547；js/data/changelog.js: v534 條目
驗證:
- 雙 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v534-teamstrip-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 255 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v533（2026-08-15）

### [v533] 改動: 回村待機鈕 hover 提示
理由: 召回語意不明。
實作:
- js/ui/hunt.js: 回村待機鈕 title（召回・休息）
- index.html: 快取 544→545；js/data/changelog.js: v533 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v533-recall-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 254 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v532（2026-08-15）

### [v532] 改動: 副本派遣鈕 hover 提示
理由: 派遣入口語意不明。
實作:
- js/ui/hunt.js: 派遣鈕 title（目的地選擇）
- index.html: 快取 543→544；js/data/changelog.js: v532 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v532-dispatch-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 253 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v531（2026-08-15）

### [v531] 改動: 地圖探索提示 hover
理由: 地圖操作引導缺說明。
實作:
- js/ui/map.js: 探索提示列 title（捲動/前往/解鎖）
- index.html: 快取 542→543；js/data/changelog.js: v531 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v531-explore-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 252 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v530（2026-08-15）

### [v530] 改動: 地圖熱區 hover 提示
理由: 地標熱區缺語意（剩餘地圖缺口）。
實作:
- js/ui/map.js: mkHit 支援 title；21 熱區（王國/10 區域/模式）
- index.html: 快取 540→541；js/data/changelog.js: v530 條目
驗證:
- 21 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v530-maphit-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 251 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v529（2026-08-15）

### [v529] 改動: 招募費用預覽列 hover 提示
理由: 費用成長規則不明。
實作:
- js/ui/hunters.js: 費用預覽 title（+20%/次）
- index.html: 快取 539→540；js/data/changelog.js: v529 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v529-costline-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 250 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v528（2026-08-15）

### [v528] 改動: 招募執行鈕 hover 提示
理由: 招募鈕語意不明。
實作:
- js/ui/hunters.js: 金幣/券/神話＋十連鈕 title
- index.html: 快取 538→539；js/data/changelog.js: v528 條目
驗證:
- 雙 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v528-recbtn-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 249 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v527（2026-08-15）

### [v527] 改動: 心願職業說明 hover 提示
理由: 心願機制範圍不明。
實作:
- js/ui/hunters.js: 心願說明 title（全招募・×2）
- index.html: 快取 537→538；js/data/changelog.js: v527 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v527-wish-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 248 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v526（2026-08-15）

### [v526] 改動: 徽章升級鈕 hover 提示
理由: 徽章升級效益不明。
實作:
- js/ui/hunters.js: 徽章升級鈕 title（消耗・倍率）
- index.html: 快取 536→537；js/data/changelog.js: v526 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v526-badge-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 247 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v525（2026-08-15）

### [v525] 改動: 換裝視窗列 hover 提示
理由: 換裝替換語意不明。
實作:
- js/ui/hunters.js: 換裝列 title（替換・戰力差・套裝）
- index.html: 快取 535→536；js/data/changelog.js: v525 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v525-pickeq-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 246 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v524（2026-08-15）

### [v524] 改動: 名冊上限列 hover 提示
理由: 名冊上限規則不明。
實作:
- js/ui/hunters.js: 名冊上限 title（酒館成長・滿員路徑）
- index.html: 快取 534→535；js/data/changelog.js: v524 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v524-roster-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 245 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。
...
理由: 解鎖目標不明。
實作:
- js/ui/hunters.js: 解鎖提示 title（技能名・訓練路徑）
- index.html: 快取 531→532；js/data/changelog.js: v521 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v521-skunlock-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 242 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v520（2026-08-15）

### [v520] 改動: 傳說被動卡 hover 提示
理由: 傳說被動強化路徑不明。
實作:
- js/ui/hunters.js: 傳說被動卡 title（永久生效・徽章）
- index.html: 快取 530→531；js/data/changelog.js: v520 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v520-legend-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 241 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v519（2026-08-15）

### [v519] 改動: 英雄詳情頭部 hover 提示
理由: 元素規則不明。
實作:
- js/ui/hunters.js: 元素/職業列 title（克制・職業語意）
- index.html: 快取 529→530；js/data/changelog.js: v519 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v519-head-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 240 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v518（2026-08-15）

### [v518] 改動: 詳情自動穿裝鈕 hover 提示
理由: 自動穿裝規則不明。
實作:
- js/ui/hunters.js: 詳情自動穿裝鈕 title
- index.html: 快取 528→529；js/data/changelog.js: v518 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v518-autoeq-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 239 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。
...
理由: 突破效益缺說明。
實作:
- js/ui/hunters.js: 突破資訊 title（+20%/階・解鎖等級）
- index.html: 快取 525→526；js/data/changelog.js: v515 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v515-promo-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 236 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v514（2026-08-15）

### [v514] 改動: 補血/補魔鈕 hover 提示
理由: 藥水操作語意不明。
實作:
- js/ui/hunters.js: 補血/補魔鈕 title
- index.html: 快取 524→525；js/data/changelog.js: v514 條目
驗證:
- 雙 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v514-potbtn-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 235 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v513（2026-08-15）

### [v513] 改動: 英雄訓練鈕 hover 提示
理由: 批量訓練停止條件不明。
實作:
- js/ui/hunters.js: 訓練×10/到滿鈕 title
- index.html: 快取 523→524；js/data/changelog.js: v513 條目
驗證:
- 雙 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v513-trainbtn-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 234 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v512（2026-08-15）

### [v512] 改動: 英雄經驗條 hover 提示
理由: 經驗效益不明。
實作:
- js/ui/hunters.js: 經驗 pbar title（升級・滿級路徑）
- index.html: 快取 522→523；js/data/changelog.js: v512 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v512-expbar-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 233 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v511（2026-08-15）

### [v511] 改動: 英雄詳情頁籤 hover 提示
理由: 頁籤內容不明。
實作:
- js/ui/hunters.js: 屬性/裝備/技能 tab title
- index.html: 快取 521→522；js/data/changelog.js: v511 條目
驗證:
- 3 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v511-detailtab-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 232 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。
...
理由: chips 語意不明。
實作:
- js/ui/hunters.js: 8 filter＋3 sort chips title
- index.html: 快取 518→520；js/data/changelog.js: v509 條目
驗證:
- 11 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v509-filtersort-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 230 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v508（2026-08-15）

### [v508] 改動: 流浪好感條 hover 提示
理由: 好感階級規則不明。
實作:
- js/ui/hunters.js: 好感進度條 title（4 階・滿好感免費）
- index.html: 快取 517→518；js/data/changelog.js: v508 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v508-favor-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 229 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v507（2026-08-15）

### [v507] 改動: 流浪招募視窗鈕 hover 提示
理由: 招募/驅逐缺前置說明。
實作:
- js/ui/hunters.js: 招募/驅逐鈕 title（好感降費・永久性）
- index.html: 快取 516→517；js/data/changelog.js: v507 條目
驗證:
- 雙 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v507-wanderrec-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 228 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v506（2026-08-15）

### [v506] 改動: 英雄視圖切換 chips hover 提示
理由: 視圖內容語意不明。
實作:
- js/ui/hunters.js: 領地/流浪視圖 chips title
- index.html: 快取 515→516；js/data/changelog.js: v506 條目
驗證:
- 2 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v506-viewchip-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 227 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v505（2026-08-15）

### [v505] 改動: 流浪英雄區標題 hover 提示
理由: 流浪機制缺規則說明。
實作:
- js/ui/hunters.js: 流浪區標題 title（來訪・好感・離開）
- index.html: 快取 514→515；js/data/changelog.js: v505 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v505-wander-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 226 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v504（2026-08-15）

### [v504] 改動: 批量遣散/驅逐鈕 hover 提示
理由: 高風險操作缺前置說明。
實作:
- js/ui/hunters.js: 批量鈕 title（雙視圖語意・返還・不可復原）
- index.html: 快取 513→514；js/data/changelog.js: v504 條目
驗證:
- 雙視圖 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v504-bulkdismiss-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 225 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v503（2026-08-15）

### [v503] 改動: 全隊訓練鈕 hover 提示
理由: 批量訓練成本不明。
實作:
- js/ui/hunters.js: 全隊訓練鈕 title（戰力排序・金幣遞增）
- index.html: 快取 512→513；js/data/changelog.js: v503 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v503-trainall-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 224 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v502（2026-08-15）

### [v502] 改動: 英雄編隊狀態列 hover 提示
理由: 待命英雄缺行動指引。
實作:
- js/ui/hunters.js: 已編隊狀態 title（酒館擴充・自動編隊）
- index.html: 快取 511→512；js/data/changelog.js: v502 條目
驗證:
- title 正確（1/5）；完整迴歸通過；rm 靜態；零 console error
- progress/v502-formed-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 223 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v501（2026-08-15）

### [v501] 改動: 英雄空狀態 hover 提示
理由: 空態缺行動指引。
實作:
- js/ui/hunters.js: 空態 title（招募路徑・自動編隊指引）
- index.html: 快取 510→511；js/data/changelog.js: v501 條目
驗證:
- 空名冊 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v501-empty-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 222 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v500（2026-08-15）

### [v500] 改動: 編隊站位標籤 hover 提示
理由: 站位機制缺說明。
實作:
- js/ui/hunters.js: 前排/後排標籤 title（承受/減傷・職業建議）
- index.html: 快取 509→510；js/data/changelog.js: v500 條目
驗證:
- 2 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v500-rowlabel-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 221 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v499（2026-08-15）

### [v499] 改動: 編隊槽格 hover 提示
理由: 站位槽格操作語意不明（原嘗試 openFormationPicker 為死碼 — 已撤銷）。
實作:
- js/ui/hunters.js: 5 槽格 title（空位/更換・立即生效）
- index.html: 快取 508→509；js/data/changelog.js: v499 條目
驗證:
- 5 title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v499-slotcell-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 220 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。

## 前輪: v498（2026-08-15）

### [v498] 改動: 編滿全部隊鈕 hover 提示
理由: 批量編隊規則不明。
實作:
- js/ui/hunters.js: 編滿全部隊鈕 title（戰力排序・不覆寫）
- index.html: 快取 507→508；js/data/changelog.js: v498 條目
驗證:
- title 正確；完整迴歸通過；rm 靜態；零 console error
- progress/v498-fillall-tip.webp
風險與回滾點: 純 title 屬性。回滾: git revert 本輪 commit。
下一輪: 預定方向 — 已 219 輪: 持續四軸輪替。診斷時開地圖看小人行走＋打一場副本看特效。
