/* v837 village-art ×3 verify */
"use strict";
const fs = require("fs");
const path = require("path");
const { chromium } = require("C:/Users/ray/AppData/Roaming/npm/node_modules/playwright");
const OUT = path.join(__dirname);
const TAG = "round-226-v837";
...
(async () => {
  ...
  await page.goto("http://127.0.0.1:8123/index.html?v=837", { waitUntil: "domcontentloaded" });
  ...
  const r = await page.evaluate(async () => {
    ...
    const ink = countColor(176, 143, 190, 157, (r, g, b) =>
      (b > r && b > 100) || (r > 100 && g > 60 && b < 90) || (r > 180 && g > 180 && b > 160));
    const mug = countColor(156, 116, 170, 132, (r, g, b) =>
      (r > 160 && g > 120 && b < 120) || (r > 180 && g > 180 && b > 160));
    const chisel = countColor(366, 116, 380, 130, (r, g, b) =>
      (r > 90 && g > 90 && b > 100 && Math.abs(r - g) < 40) || (r > 100 && g > 60 && b < 90));

    st.settings.reducedMotion = true;
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h1a = hashRoi(176, 143, 190, 157);
    const h1b = hashRoi(156, 116, 170, 132);
    const h1c = hashRoi(366, 116, 380, 130);
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h2a = hashRoi(176, 143, 190, 157);
    const h2b = hashRoi(156, 116, 170, 132);
    const h2c = hashRoi(366, 116, 380, 130);

    return { ink, mug, chisel, rmSame: h1a === h2a && h1b === h2b && h1c === h2c };
  });
  ...
  const asserts = [
    { name: "srcInk", ok: k.includes("v837 圖書館墨水瓶") },
    { name: "srcMug", ok: k.includes("v837 酒館酒杯") },
    { name: "srcChisel", ok: k.includes("v837 寶石鑿刀") },
    { name: "inkROI", ok: r.ink >= 6 },
    { name: "mugROI", ok: r.mug >= 6 },
    { name: "chiselROI", ok: r.chisel >= 6 },
