/* v745 village-art ×3 verify */
"use strict";
const fs = require("fs");
const path = require("path");
const { chromium } = require("C:/Users/ray/AppData/Roaming/npm/node_modules/playwright");
const OUT = path.join(__dirname);
const TAG = "round-134-v745";
...
(async () => {
  ...
  await page.goto("http://127.0.0.1:8123/index.html?v=745", { waitUntil: "domcontentloaded" });
  ...
  const r = await page.evaluate(async () => {
    ...
    // 銀碗／金供
    const bowl = countColor(316, 146, 328, 158, (r, g, b) =>
      (r > 150 && g > 150 && b > 160 && Math.abs(r - g) < 30) || (r > 200 && g > 160 && b < 120));
    // 捲軸米褐
    const scroll = countColor(154, 148, 166, 158, (r, g, b) =>
      r > 180 && g > 160 && b > 120 && b < 200 && r >= g && g > b);
    // 酒桶褐
    const keg = countColor(146, 96, 158, 110, (r, g, b) =>
      r > 90 && r < 180 && g > 50 && g < 130 && b < 100 && r > g && g >= b);

    st.settings.reducedMotion = true;
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h1 = hashRoi(140, 90, 330, 165);
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h2 = hashRoi(140, 90, 330, 165);

    return { bowl, scroll, keg, rmSame: h1 === h2, h1, h2 };
  });
  ...
  assert("srcBowl", k.includes("v745 祭壇銀供碗"));
  assert("srcScroll", k.includes("v745 圖書館捲軸堆"));
  assert("srcKeg", k.includes("v745 酒館橡木酒桶"));
  assert("bowlROI", r.bowl >= 4);
  assert("scrollROI", r.scroll >= 6);
  assert("kegROI", r.keg >= 8);
  ...
  const table = [
    "=== 村莊美術驗證 ===",
    "bowl=" + r.bowl + " scroll=" + r.scroll + " keg=" + r.keg + " rmSame=" + r.rmSame,
