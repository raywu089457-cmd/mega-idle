/* v781 village-art ×3 verify */
"use strict";
const fs = require("fs");
const path = require("path");
const { chromium } = require("C:/Users/ray/AppData/Roaming/npm/node_modules/playwright");
const OUT = path.join(__dirname);
const TAG = "round-170-v781";
...
(async () => {
  ...
  await page.goto("http://127.0.0.1:8123/index.html?v=781", { waitUntil: "domcontentloaded" });
  ...
  const r = await page.evaluate(async () => {
    ...
    // bread board ~156,112
    const bread = countColor(152, 108, 170, 124, (r, g, b) =>
      (r > 160 && g > 100 && b < 120 && r > b) || (r > 90 && g > 50 && b < 90 && r > g));
    // knocker ~88,98
    const knock = countColor(84, 94, 100, 110, (r, g, b) =>
      (Math.abs(r - g) < 25 && Math.abs(g - b) < 25 && r > 70 && r < 180));
    // pouch ~400,138
    const pouch = countColor(396, 134, 412, 150, (r, g, b) =>
      (r > 180 && g > 140 && b < 140) || (r > 90 && g > 50 && b < 90 && r > b));

    st.settings.reducedMotion = true;
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h1a = hashRoi(152, 108, 170, 124);
    const h1b = hashRoi(84, 94, 100, 110);
    const h1c = hashRoi(396, 134, 412, 150);
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h2a = hashRoi(152, 108, 170, 124);
    const h2b = hashRoi(84, 94, 100, 110);
    const h2c = hashRoi(396, 134, 412, 150);

    return { bread, knock, pouch, rmSame: h1a === h2a && h1b === h2b && h1c === h2c };
  });
  ...
  const asserts = [
    { name: "srcBread", ok: k.includes("v781 酒館麵包板") },
    { name: "srcKnock", ok: k.includes("v781 王城門環") },
    { name: "srcPouch", ok: k.includes("v781 市場錢袋") },
    { name: "breadROI", ok: r.bread >= 6 },
    { name: "knockROI", ok: r.knock >= 6 },
    { name: "pouchROI", ok: r.pouch >= 6 },
