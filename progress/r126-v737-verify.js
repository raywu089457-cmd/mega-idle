/* v737 village-art ×3 verify */
"use strict";
const fs = require("fs");
const path = require("path");
const { chromium } = require("C:/Users/ray/AppData/Roaming/npm/node_modules/playwright");
const OUT = path.join(__dirname);
const TAG = "round-126-v737";
...
(async () => {
  ...
  await page.goto("http://127.0.0.1:8123/index.html?v=737", { waitUntil: "domcontentloaded" });
  ...
  const r = await page.evaluate(async () => {
    ...
    // 木牌褐
    const sign = countColor(150, 88, 172, 108, (r, g, b) =>
      r > 100 && r < 220 && g > 60 && g < 170 && b < 100 && r > g);
    // 金字
    const gold = countColor(150, 88, 172, 108, (r, g, b) =>
      r > 180 && g > 140 && b < 140 && r >= g);
    // 研缽灰
    const mortar = countColor(88, 142, 110, 162, (r, g, b) =>
      Math.abs(r - g) < 30 && Math.abs(g - b) < 30 && r > 90 && r < 190);
    // 天秤銅
    const scale = countColor(382, 140, 408, 160, (r, g, b) =>
      r > 140 && g > 80 && g < 180 && b < 100 && r > g);

    st.settings.reducedMotion = true;
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h1 = hashRoi(80, 80, 420, 170);
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h2 = hashRoi(80, 80, 420, 170);

    return { sign, gold, mortar, scale, rmSame: h1 === h2, h1, h2 };
  });
  ...
  assert("srcSign", k.includes("v737 酒館木牌"));
  assert("srcMortar", k.includes("v737 煉金研缽"));
  assert("srcScale", k.includes("v737 市場天秤"));
  assert("signROI", r.sign >= 8);
  assert("goldROI", r.gold >= 2);
  assert("mortarROI", r.mortar >= 6);
  assert("scaleROI", r.scale >= 6);
  ...
  const table = [
    "=== 村莊美術驗證 ===",
    "sign=" + r.sign + " gold=" + r.gold + " mortar=" + r.mortar + " scale=" + r.scale + " rmSame=" + r.rmSame,
