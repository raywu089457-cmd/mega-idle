/* v749 village-art ×3 verify */
"use strict";
const fs = require("fs");
const path = require("path");
const { chromium } = require("C:/Users/ray/AppData/Roaming/npm/node_modules/playwright");
const OUT = path.join(__dirname);
const TAG = "round-138-v749";
...
(async () => {
  ...
  await page.goto("http://127.0.0.1:8123/index.html?v=749", { waitUntil: "domcontentloaded" });
  ...
  const r = await page.evaluate(async () => {
    ...
    // 水桶藍
    const bucket = countColor(216, 98, 228, 110, (r, g, b) =>
      b > r && b > g && b > 120 && g > 80);
    // 掛鉗銀灰
    const tongs = countColor(296, 88, 308, 102, (r, g, b) =>
      Math.abs(r - g) < 25 && Math.abs(g - b) < 25 && r > 80 && r < 180);
    // 價目板米字／金點
    const board = countColor(388, 144, 402, 158, (r, g, b) =>
      (r > 180 && g > 160 && b > 140 && r >= g) || (r > 200 && g > 160 && b < 130));

    st.settings.reducedMotion = true;
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h1 = hashRoi(210, 85, 405, 165);
    if (MG.ui.kingdom.raf) MG.ui.kingdom.raf(6000);
    const h2 = hashRoi(210, 85, 405, 165);

    return { bucket, tongs, board, rmSame: h1 === h2, h1, h2 };
  });
  ...
  assert("srcBucket", k.includes("v749 訓練場水桶"));
  assert("srcTongs", k.includes("v749 鐵匠掛鉗"));
  assert("srcBoard", k.includes("v749 市場價目板"));
  assert("bucketROI", r.bucket >= 4);
  assert("tongsROI", r.tongs >= 6);
  assert("boardROI", r.board >= 6);
  ...
  const table = [
    "=== 村莊美術驗證 ===",
    "bucket=" + r.bucket + " tongs=" + r.tongs + " board=" + r.board + " rmSame=" + r.rmSame,
