...
- 建築曲線：Lv≤12 維持 ×2.1-2.3/級原曲線（與收入對齊）；Lv13+ 經 v553 阻尼（×1.35/級）後於 v624 再修正 — ×1.35 在 Lv25-40 複利 ×222 遠超近似線性的收入成長（r10 收入 524萬/h 下 altar Lv30 單級 33.6 天、castle Lv60 深尾 10046 天，階梯二次凍結）→ 改 ×1.20/級＋指數段封頂 30 級＋線性尾 ×(1+0.3×超出級數)，單級回落到小時級（主線建築）～1-4 天（深尾尊貴建築），castle Lv60 滿級 ≈8.5 天等值；Lv≤12 段逐位元不變。v631：×1.20 校準收入锚高估（用 r10 524萬/h，實際推段玩家 1.86-3.02M/h）→ ×1.12；祭壇 Lv25 單級 2.04億→8,315萬（4.6天→1.9天 @1.86M/h）、圖書館 Lv25 回小時級（12.3h）；深尾同步係數 ×(1.12/1.2)^30≈0.13，castle Lv43→60 累計仍 ≈74 天 @8M/h（尊貴可達不變）
- 未動：魔物 TB（後期牆可被 5 人編隊+強化+突破+套裝突破）、職業數值、經驗曲線。

## 13. 副本難度系統（2026-08-09）

- 4 難度（`MG.config.DIFFICULTY`，存於 `hunt.difficulty`，舊存檔遷移=0）：普通 ×1.0 / 困難 ×1.8 / 地獄 ×3.2 / 夢魘 ×5.5（魔物三圍）；獎勵金幣 = 經驗 = 難度倍率 ×1/1.8/3.2/5.5（v204 parity — 金/秒與經驗/秒在高難度效率 =1，非自我懲罰；防禦不隨難度縮放）。
- **v583 掉落 parity**：每殺掉落機率（裝備/寶石/技能書/素材/藥水/BOSS 額外券・書）同乘難度倍率 `d.mult`（clamp 0.95；BOSS 必掉裝備與首殺榮譽、BOSS 必掉寶石不乘 — 必掉語義與難度無關）— 與 v204 金/經 parity 對稱，金/經/掉落三軸全 parity；掉落一覽（`dropInfoOf`/`lootInfoBlock`）與 `rollKill` 同源顯示難度倍率後的機率。
- 難度門控：困難需抵達第 3 區域、地獄第 5、夢魘第 7（`maxTierReached`）；未解鎖點擊顯示鎖定提示。
- 切換難度：`battle.reset()` + 清除 `pendingHp`（新難度=新首領戰）；`scaledMonster` 統一乘難度倍率（離線/掉落/建議戰力全部自動繼承）。
...
- **持續性生命（HP）系統**：獵人擁有持久 HP（`hunter.hp`，舊檔自動滿血遷移），開戰承接當前血量、每 tick 寫回。
  ...
  - 切換獵場/難度/召回**都不補血**；**升級自動補滿**生命、突破按比例成長。
  - **切換鎖**：戰鬥進行中（phase=fight）不可切換獵場或難度，需等當前戰鬥結束。
  - **編輯鎖**：戰鬥進行中鎖定僅針對**參戰（派遣中）英雄**（突破/訓練/遣散/穿戴/卸下不可；其裝備不可分解/鑲嵌）— **板凳英雄照常可編輯**（招募也一直可用）；編隊編輯（編入/移出/自動編隊）戰鬥中一律鎖定。`U.fightGuard(h)` 依參戰與否判斷。
  - 無 MP 系統（技能為冷卻制）。
- 普通難度曲線重調（無作弊真人實測發現 r2 死區）：r2-r4 魔物三圍 -10~15%、r1-r4 經驗 +50% → lvl 3-5 隊可連續討伐 r2 首領（實測 10 分鐘 +21 殺/+16 首領，先前同階段 15 分鐘僅 +2 殺）。
